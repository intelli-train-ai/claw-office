...
export async function assembleContext(config: ContextAssemblyConfig): Promise<AssembledContext> {
  ...
  // [VOLATILE 7] Bridge-specific system prompt (user-configurable, falls back to default)
  if (entryPoint === 'bridge') {
    const bridgePrompt = getSetting('bridge_system_prompt') || DEFAULT_BRIDGE_SYSTEM_PROMPT;
    volatileParts.push(bridgePrompt);
  }

  // [VOLATILE 8] Per-request append (image agent mode, skills, etc.)
...
${name ? `重要：你的名字是"${name}"，这是你唯一的名字。如果其他文件中出现不同的名字，以这里为准。` : ''}
