...
| `im:chat:read` | 读取群聊信息 |
| `im:resource` | 获取与上传图片或文件资源（收发图片、文件、视频） |
...
流程与国内版相同，但在 [Lark Developer](https://open.larksuite.com) 上操作。在 SafeClaw 配置时选择域名为 **Lark**。
...
3. 检查 SafeClaw 日志中是否有 `[feishu/gateway] handleEventData type: card` 输出

### 流式卡片不显示

1. 确认已申请 `cardkit:card:write` 和 `cardkit:card:read` 权限
2. 确认应用已重新发布（权限变更后需要重新发布）
3. 检查 SafeClaw 日志中是否有 `[card-controller]` 相关输出
