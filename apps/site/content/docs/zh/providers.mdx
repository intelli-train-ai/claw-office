---
title: 服务商配置
description: 配置 LLM 服务商以驱动 SafeClaw。
---

# 服务商配置

SafeClaw 支持多种 LLM 服务商。你可以同时配置多个服务商，在不同对话中使用不同的模型。

## 认证方式概览

SafeClaw 有两种获取 API 凭据的途径：

### 1. CLI 环境认证（自动检测）

如果你在 shell 环境中设置了 `ANTHROPIC_API_KEY` 或 `ANTHROPIC_AUTH_TOKEN` 环境变量，SafeClaw 启动时会**自动检测**并作为内置服务商使用。设置中心也会检查这些凭据，如果检测到会自动将服务商步骤标记为已完成。

```bash
export ANTHROPIC_API_KEY="sk-ant-..."
```

> **注意：** 通过 `claude config set` 或 Claude Code 的 `/config` 命令切换的配置**不会被 SafeClaw 识别**。SafeClaw 只读取 shell 环境变量，不共享 Claude Code CLI 的内部配置。如果你在 CLI 中通过 `cc switch` 或类似方式切换了账号/密钥，需要在 SafeClaw 的 **设置 > 服务商** 中重新手动配置对应的密钥。

> 修改环境变量后需要**重启 SafeClaw** 才能生效。

### 2. 手动添加服务商

在 **设置 > 服务商** 中手动添加 API 密钥。这些凭据存储在 SafeClaw 本地数据库中，与 CLI 环境相互独立。

适合需要使用多个服务商、或使用非 Anthropic 服务的场景。

### 优先级

当发送消息时，SafeClaw 按以下顺序确定使用哪个服务商：
...
SafeClaw 内置了国内主流服务商的配置预设，选择后自动填充基础 URL 和默认模型：
...
在 SafeClaw 中添加国内服务商时，系统会自动处理认证方式，你只需填写对应平台提供的密钥。每个服务商卡片上都有直接获取 API Key 的链接。
...
- **说明**：在 SafeClaw 中添加后，系统会读取你的 AWS 环境变量进行认证。不需要在界面中填写密钥。
...
通过 Ollama 运行本地模型。Ollama 提供了 Anthropic 兼容的 API，SafeClaw 可以直接连接。
...
### 已设置环境变量但 SafeClaw 没有检测到

- 确认环境变量在 SafeClaw 启动时的 shell 环境中可用
- 如果通过 `.zshrc` / `.bashrc` 设置，确保修改后**重启了 SafeClaw**（不是刷新）
...
> **模型名称很重要**：在 SafeClaw 中填写的模型名称必须与 `ollama list` 显示的名称**完全一致**（包括冒号后的标签部分）。例如 `gemma4:e4b`，不能只写 `gemma4`。

更多模型和用法参见 [Ollama 官方文档](https://docs.ollama.com/)。

### 第二步：在 SafeClaw 中添加 Ollama
