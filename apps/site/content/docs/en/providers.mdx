...
description: Configure LLM providers to power SafeClaw.
---

# Provider Configuration

SafeClaw supports multiple LLM providers. You can configure several providers simultaneously and use different models in different conversations.

## Authentication Overview

SafeClaw has two ways to obtain API credentials:

### 1. CLI Environment Authentication (Auto-Detected)

If you have the `ANTHROPIC_API_KEY` or `ANTHROPIC_AUTH_TOKEN` environment variable set in your shell, SafeClaw **automatically detects** it on startup and uses it as a built-in provider. The Setup Center also checks for these credentials and marks the provider step as complete if found.

```bash
export ANTHROPIC_API_KEY="sk-ant-..."
```

> **Note:** Configurations changed via `claude config set` or Claude Code's `/config` command are **not recognized by SafeClaw**. SafeClaw only reads shell environment variables and does not share Claude Code CLI's internal configuration. If you switched accounts/keys in the CLI via `cc switch` or similar methods, you need to manually reconfigure the corresponding key in SafeClaw's **Settings > Providers**.

> After modifying environment variables, you need to **restart SafeClaw** for changes to take effect.

### 2. Manually Adding Providers

Manually add API keys in **Settings > Providers**. These credentials are stored in SafeClaw's local database, independent of the CLI environment.

This is ideal for scenarios where you need multiple providers or non-Anthropic services.

### Priority

When sending a message, SafeClaw determines which provider to use in the following order:
...
SafeClaw includes built-in configuration presets for major Chinese providers. After selecting one, the Base URL and default model are auto-filled:
...
When adding a Chinese provider in SafeClaw, the system automatically handles the authentication method — you just need to enter the key provided by the respective platform. Each provider card shows a direct link to obtain your API key.
...
- **Note**: After adding in SafeClaw, the system reads your AWS environment variables for authentication. No need to enter keys in the UI.
...
Run local models through Ollama. Ollama provides an Anthropic-compatible API that SafeClaw can connect to directly.
...
### Environment variables are set but SafeClaw doesn't detect them

- Confirm the environment variables are available in the shell environment when SafeClaw starts
- If set via `.zshrc` / `.bashrc`, make sure you **restarted SafeClaw** (not just refreshed) after the change
...
> **Model names matter**: The model name you enter in SafeClaw must **exactly match** the name shown by `ollama list` (including the tag after the colon). For example, `gemma4:e4b` — not just `gemma4`.

For more models and usage, see the [Ollama documentation](https://docs.ollama.com/).

### Step 2: Add Ollama in SafeClaw
