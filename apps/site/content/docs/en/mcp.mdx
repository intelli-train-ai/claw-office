...
[Model Context Protocol (MCP)](https://modelcontextprotocol.io) is an open standard for connecting AI assistants to external tools. SafeClaw has a built-in MCP client that supports connecting to any MCP server to extend Claude's capabilities.
