---
title: FAQ
description: Frequently asked questions about SafeClaw.
---
...
SafeClaw is a desktop workspace for Claude Code. It provides a graphical interface on top of the Claude Code CLI, integrating multi-provider management, MCP plugins, skills, message bridging, and the assistant workspace. Built with Electron + Next.js.

### What is the relationship between SafeClaw and Claude Code CLI?

SafeClaw is a graphical frontend for the Claude Code CLI. It calls Claude Code CLI capabilities through the Claude Agent SDK while adding GUI-exclusive features on top (multi-provider switching, bridging, gallery, etc.). You need to install the Claude Code CLI before using SafeClaw.

### Is SafeClaw free?

SafeClaw is open source and free to use. You need your own API key from a supported provider (Anthropic, OpenRouter, or others). API usage fees are charged by the provider.
...
SafeClaw requires Node.js 18+. The first-launch setup wizard will detect this and offer automatic installation. You can also install manually from [nodejs.org](https://nodejs.org).
...
2. If installed but not detected, the binary may not be in SafeClaw's PATH. Try launching SafeClaw from the terminal (`open /Applications/SafeClaw.app` on macOS) so it inherits your shell environment
...
Go to **System Settings > Privacy & Security**, find the SafeClaw prompt, and click "Open Anyway".
...
Use the [Message Bridge](/docs/bridge) feature to connect SafeClaw to Telegram, Discord, Feishu, or QQ. Keep the desktop app running and you can chat with Claude from your phone.
...
Open an issue on [GitHub Issues](https://github.com/intelli-train-ai/claw-office/issues). Please include:

- Your OS and SafeClaw version
- Steps to reproduce
- Relevant error logs
