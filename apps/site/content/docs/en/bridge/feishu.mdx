...
| `im:resource` | Upload and download images, files, and videos in messages |
...
The process is the same as the domestic edition, but performed on [Lark Developer](https://open.larksuite.com). When configuring in SafeClaw, select the domain as **Lark**.

## Configure in SafeClaw
...
3. Check SafeClaw logs for `[feishu/gateway] handleEventData type: card` output
...
3. Check SafeClaw logs for `[card-controller]` related output
