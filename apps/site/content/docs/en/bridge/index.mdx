...
The Message Bridge lets you chat with Claude from instant messaging apps on your phone. Start SafeClaw on your desktop, configure a Bot, and you can interact with Claude anytime through Telegram, Discord, Feishu / Lark, or QQ.
...
```
Phone IM → Bot receives message → SafeClaw forwards to Claude → Claude replies → Bot sends reply back to IM
```

1. You send a message to the Bot in your messaging platform
2. SafeClaw's Bridge service receives the message and forwards it to Claude
...
- **Auto-start** — Automatically start the bridge service when SafeClaw launches
...
You can use slash commands in the IM to control SafeClaw:

| Command | Arguments | Description |
|---------|-----------|-------------|
| `/new` | `[path]` | Start a new conversation, optionally specifying a working directory |
| `/bind` | `<session-id>` | Bind the current IM chat to an existing SafeClaw conversation |
...
- **Keep SafeClaw running** — The bridge relies on the desktop app as a relay; the Bot cannot respond if SafeClaw is closed
