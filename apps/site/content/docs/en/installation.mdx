---
title: Installation
description: How to install and set up SafeClaw on your system.
---
...
Download the latest release for your platform from [GitHub Releases](https://github.com/intelli-train-ai/claw-office/releases).
...
2. Open the DMG and drag SafeClaw to your Applications folder
3. Launch SafeClaw from Applications
...
3. SafeClaw will be available in your Start menu

## First Launch & Setup Center

On first launch, SafeClaw automatically opens the **Setup Center** — a guided setup overlay that walks you through three prerequisites:
...
- **Multiple installations detected** — If SafeClaw finds more than one Claude Code binary (e.g., one installed via npm and another natively), it shows a warning with cleanup instructions. See [Claude Code Conflicts](#claude-code-conflicts) below.
...
> **Note:** Skipping the provider step doesn't mean you have a working provider. If you try to send a message without any configured provider, SafeClaw will prompt you to set one up.
...
### How SafeClaw detects conflicts
...
SafeClaw needs at least one API provider to function. Go to **Settings** and open the **Providers** section to add one:
...
SafeClaw includes an auto-updater. When a new version is available, you'll see a notification in the app. You can also manually check for updates in the **General** section of **Settings**.
