<img src="docs/icon-readme.png" width="32" height="32" alt="SafeClaw" style="vertical-align: middle; margin-right: 8px;" /> SafeClaw
===
...
**どこからでも制御。** Bridge で SafeClaw を Telegram、Feishu、Discord、QQ に接続。スマートフォンからメッセージを送り、デスクトップで返答を受け取れます。
...
2. 認証: `claude login`
3. [Releases](https://github.com/intelli-train-ai/claw-office/releases) ページからプラットフォームに合ったインストーラーをダウンロード
4. SafeClaw を起動
...
4. **Assistant Workspace を設定**（任意）-- **設定 > Assistant** でワークスペースディレクトリを選択し、Onboarding を有効化。SafeClaw がワークスペースルートに `soul.md`、`user.md`、`claude.md`、`memory.md` を作成（状態は `.assistant/` サブディレクトリに保存）。
...
[Releases](https://github.com/intelli-train-ai/claw-office/releases) ページからダウンロードしてください。
...
**オプション 1** -- Finder で `SafeClaw.app` を右クリック > 開く > 確認。
...
📖 **完全ドキュメント:** [English](https://www.safeclaw.sh/docs) | [中文](https://www.safeclaw.sh/zh/docs)

**はじめに:**
- [クイックスタート](#クイックスタート) -- ダウンロードまたはソースからビルド
- [初回起動](#初回起動) -- 認証、プロバイダー設定、ワークスペースセットアップ
- [インストールガイド](https://www.safeclaw.sh/docs/installation) -- 詳細なセットアップ手順

**ユーザーガイド:**
- [Providers](https://www.safeclaw.sh/docs/providers) -- Anthropic、OpenRouter、Bedrock、Vertex、カスタムエンドポイントの設定
- [MCP サーバー](https://www.safeclaw.sh/docs/mcp) -- Model Context Protocol サーバーの追加と管理
- [Skills](https://www.safeclaw.sh/docs/skills) -- カスタムスキル、プロジェクトスキル、skills.sh マーケットプレイス
- [Bridge](https://www.safeclaw.sh/docs/bridge) -- Telegram、Feishu、Discord、QQ によるリモート制御
- [Assistant Workspace](https://www.safeclaw.sh/docs/assistant-workspace) -- ペルソナファイル、Onboarding、メモリ、デイリーチェックイン
- [FAQ](https://www.safeclaw.sh/docs/faq) -- よくある質問と解決方法
...
```
`claude login` で認証を完了し、`claude --version` が動作することを確認してから SafeClaw を起動してください。
</details>
...
ワークスペース設定後、SafeClaw は**ワークスペースルートディレクトリ**に 4 つの Markdown ファイルを作成: `soul.md`（パーソナリティ）、`user.md`（ユーザープロファイル）、`claude.md`（ルール）、`memory.md`（長期メモ）。状態管理（Onboarding 進捗、チェックイン日付）は `.assistant/` サブディレクトリに保存。デイリーメモリは `memory/daily/` に保存。
</details>
...
- [GitHub Issues](https://github.com/intelli-train-ai/claw-office/issues) -- バグ報告と機能リクエスト
- [GitHub Discussions](https://github.com/intelli-train-ai/claw-office/discussions) -- 質問と一般的なディスカッション
...
- チャットデータは `~/.safeclaw/safeclaw.db`（開発モード: `./data/`）に保存
